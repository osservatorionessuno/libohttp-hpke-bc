...
class OhttpRoundTripTest {
    ...
    fun `request and response round trip`() {
        ...
        val config = byteArrayOf(0x2A) + u16(KEM_X25519_HKDF_SHA256) + publicKey +
        ...
        val encResponse = responseNonce + aesGcmSeal(hkdfExpand(prk, "key".toByteArray(), nk), hkdfExpand(prk, "nonce".toByteArray(), nn), response)
    ...
    private fun hkdfExtract(salt: ByteArray, ikm: ByteArray) =
        Mac.getInstance("HmacSHA256").run { init(SecretKeySpec(salt, "HmacSHA256")); doFinal(ikm) }

    private fun hkdfExpand(prk: ByteArray, info: ByteArray, length: Int): ByteArray {
        val mac = Mac.getInstance("HmacSHA256").apply { init(SecretKeySpec(prk, "HmacSHA256")) }
        val out = ByteArray(length); var t = ByteArray(0); var filled = 0; var counter = 1
        while (filled < length) {
            mac.update(t); mac.update(info); mac.update(counter.toByte()); t = mac.doFinal()
            val take = minOf(t.size, length - filled); t.copyInto(out, filled, 0, take); filled += take; counter++
        }
        return out
    }

    private fun aesGcmSeal(key: ByteArray, nonce: ByteArray, pt: ByteArray) =
        Cipher.getInstance("AES/GCM/NoPadding").run {
            init(Cipher.ENCRYPT_MODE, SecretKeySpec(key, "AES"), GCMParameterSpec(128, nonce)); doFinal(pt)
        }
}
